...
(defvar cana-layouts nil "かな入力配列の連想配列.")
...
(defvar cana-previous-layout nil
  "直前に選択されていたかな入力配列.")

(defun cana-set-layout (layout)
  "かな入力配列 LAYOUT に設定する."
  (let ((name (car layout))
        (rules (cddr layout)))
    (setq cana-rules (copy-alist rules))
    ;; ASCII 文字列の長さで降順ソートしておく
    (setq cana-rules
          (sort cana-rules
                (lambda (a b) (< (length (car b)) (length (car a))))))
    (setq cana-rules-pattern
          (concat "\\(.*?\\)\\("
                  (mapconcat (lambda (s) (regexp-quote s))
                             (mapcar (lambda (rule) (car rule)) cana-rules)
                             "\\|")
                  "\\)\\(.*\\)"))
    (if (not (equal name (car cana-current-layout)))
...
(defun cana-select-layout ()
  ...
  (when (equal current-input-method "cana")
    (let* ((prev-name (car cana-previous-layout))
           (curr-name (car cana-current-layout))
           (all-names (mapcar #'car cana-layouts))
           (names nil)
           ...
           (name (ido-completing-read
                  "Layout: " names nil t))
           (layout (assoc name cana-layouts)))
...
(defvar ckc-keymap
  (let ((map (make-sparse-keymap))
    ...
    (define-key map "\C-p" 'ckc-prev)
    ;; (define-key map "\C-c" 'ckc-cancel)
    (define-key map "\C-c" 'ckc-show-conversion-list-or-prev-group)
    (define-key map "\C-?" 'ckc-cancel)
    (define-key map "\C-f" 'ckc-next-phrase)
    (define-key map [?\C- ] 'ckc-first-char-only)
    (define-key map [delete] 'ckc-cancel)
